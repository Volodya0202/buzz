...
export function SearchDialogInputRow({
  ...
  return (
    ...
    >
      ...
      {scopeLabel ? (
        <button
          aria-label={`Убрать область поиска: ${scopeLabel}`}
          ...
          title={`Убрать область поиска: ${scopeLabel}`}
          type="button"
        >
      ...
      <div className="relative min-w-0 flex-1">
        {query.length === 0 ? (
          <span className="pointer-events-none absolute inset-y-0 left-0 flex items-center text-base leading-none">
            {scopeLabel ? (
              <span className="text-muted-foreground">Поиск сообщений</span>
            ) : (
    ...
    <div className="px-3 py-3.5">
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          ...
          >
            {isDirectMessage ? "Поиск в переписке с " : "Поиск в "}
