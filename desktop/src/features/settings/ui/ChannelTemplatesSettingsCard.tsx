...
export function ChannelTemplatesSettingsCard() {
  ...
  function handleDelete() {
    ...
    deleteMutation.mutate(deleteTarget.id, {
      onSuccess: () => {
        toast.success(`Удалён шаблон "${deleteTarget.name}"`);
  ...
  return (
    <section className="min-w-0" data-testid="settings-channel-templates">
      <SettingsSectionHeader
        ...
        description={
          <>
            Сохраняйте повторяющиеся конфигурации каналов и применяйте их при
      ...
      {templatesQuery.isLoading ? (
        <SettingsOptionGroup title="Шаблоны">
        ...
        <SettingsOptionGroup title="Шаблоны">
          <div className="px-4 py-8 text-center text-sm text-muted-foreground">
            Шаблонов пока нет. Создайте шаблон, чтобы сохранить готовую
            конфигурацию канала.
          </div>
        </SettingsOptionGroup>
      ) : (
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (isEditing) {
      ...
      updateMutation.mutate(input, {
        onSuccess: () => {
          toast.success(`Шаблон "${trimmedName}" обновлён`);
          onOpenChange(false);
        },
        onError: (error) => {
          toast.error(
      ...
      createMutation.mutate(input, {
        onSuccess: (created) => {
          toast.success(`Шаблон "${trimmedName}" создан`);
          onCreated?.(created);
          onOpenChange(false);
        },
        onError: (error) => {
      ...
      <ChooserDialogContent
        ...
        description={
          isEditing
            ...
            : "Сохраните готовую конфигурацию канала."
        }
        footer={
        ...
        <form className="space-y-5" id="template-form" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="Для чего предназначен этот шаблон"
              rows={2}
              value={description}
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <p
              ...
              data-settings-subcopy
            >
              Используйте {"{channel.name}"} и {"{template.name}"} в качестве подстановок.
            </p>
          </div>
    ...
    <div className="space-y-3">
      <div>
        ...
        <p
          ...
          data-settings-subcopy
        >
          Выберите команды для включения в этот шаблон.
        </p>
      </div>
    ...
    <div className="space-y-3">
      <div>
        ...
        <p
          ...
          data-settings-subcopy
        >
          Выберите среду выполнения для каждого агента.
        </p>
      </div>
      ...
      {providersLoading ? (
        ...
        <p className="text-xs text-muted-foreground">
          Среды выполнения ACP не обнаружены. Установите хотя бы одну среду, чтобы назначить её.
        </p>
      ) : (
