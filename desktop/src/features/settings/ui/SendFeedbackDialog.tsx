...
const FEEDBACK_CATEGORIES: readonly FeedbackCategory[] = [
  ...
  { id: "praise", label: "Благодарность", icon: ThumbsUp, positive: true },
  { id: "needs-work", label: "Нужна доработка", icon: Wrench },
];
...
export function SendFeedbackDialog({
  ...
  async function attachImage() {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Не удалось прикрепить изображение.",
      );
    }
  }
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      ...
      >
        <DialogHeader className="space-y-0 pb-5">
          ...
          >
            Отзыв отправляется конфиденциально в эту систему Buzz и не публикуется в канале. Вложения загружаются перед отправкой.
          </p>
        </DialogHeader>
          ...
          */}
          ...
          <div className="flex items-stretch gap-3">
            <Textarea
              ...
              placeholder="Расскажите нам, что пошло не так, или поделитесь общим отзывом."
              value={message}
            />
            ...
            {resolvedAttachedImageUrl ? (
              <div className="group/attachment relative flex w-32 shrink-0 flex-col overflow-hidden rounded-lg border border-border/70 bg-muted/40">
                <button
                  aria-label="Просмотр прикреплённого изображения"
          ...
          <div className="mt-4 space-y-1.5">
            ...
            <p className="pl-6 text-xs text-muted-foreground">
              Включает время создания, версию приложения, платформу, user agent и язык. Строки журналов приложения не собираются.
            </p>
          </div>
      ...
      {resolvedAttachedImageUrl ? (
        <Dialog onOpenChange={setPreviewOpen} open={previewOpen}>
          ...
          >
            <DialogTitle className="sr-only">Прикреплённое изображение</DialogTitle>
            <img
              alt="Attached"
